...
class Thumbnail
{
   ...
   public static function resizeImage($name,$sourceFile, $width, $height)//thuc muc/thu muc lưu/ do rong
    {
        $File=$sourceFile.'/'.$width;
        UploadImage::SetFile($File);
        $destFile=$sourceFile.'/'.$width.'/'.$name;
...
# This is the resizing/resampling/transparency-preserving magic
        ...
        if (($info[2] == IMAGETYPE_GIF) || ($info[2] == IMAGETYPE_PNG))
        {
            ...
            if ($transparency >= 0)
            {
                $transparent_color = imagecolorsforindex($image);
